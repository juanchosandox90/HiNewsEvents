...
class EconomicsFragment : Fragment() {
    ...
    private var newsViewModel: EconomicsViewModel? = null
    private var newsListAdapter: EconomicsNewsAdapter? = null
    ...
    override fun onCreateView(
        ...
        newsViewModel = ViewModelProvider(this).get(EconomicsViewModel::class.java)
    ...
    private fun initAdapter() {
        ...
        newsListAdapter = EconomicsNewsAdapter { newsViewModel?.retry() }
        recyclerView.layoutManager = LinearLayoutManager(context, LinearLayoutManager.VERTICAL, false)
        recyclerView.adapter = newsListAdapter

        newsViewModel?.newsList?.observe(viewLifecycleOwner, Observer {
            Log.i(TAG, "initAdapter : economicsNewsViewModel?.newsList?.observe")
            if(newsViewModel!!.listIsEmpty()){
                Log.d(TAG, "initState economicsNewsViewModel!!.listIsEmpty : showErrorLayout : errorMsg : " + newsViewModel?.getErrorMsg())
                showErrorLayout()
                Utils.showToastMessage(context,"Could Not Get Data From API!\nSometimes HTTP 429 Too Many Requests")
            }else{
                Log.d(TAG, "initState economicsNewsViewModel!!.listIsEmpty : nofityItemChange and hideErrorLayout")
    ...
    class EconomicsNewsAdapter(private val retry: () -> Unit) :
        PagedListAdapter<Article, EconomicsNewsAdapter.ViewHolder>(NewsDiffCallback) {
