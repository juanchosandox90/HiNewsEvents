...
import com.huawei.hinewsevents.data.viewmodel.FinanceViewModel
import com.huawei.hinewsevents.utils.extension.Utils

class FinanceFragment : Fragment() {

    val TAG: String = FinanceFragment::class.simpleName.toString()
    ...
    private var newsViewModel: FinanceViewModel? = null
    private var newsListAdapter: FinanceNewsAdapter? = null
    ...
    override fun onCreateView(
        ...
        swipeRefreshLayout.setOnRefreshListener {
            if( Utils.haveNetworkConnection(view.context) ) {
                ...
                newsViewModel = ViewModelProvider(this).get(FinanceViewModel::class.java)
                getViewModelAndSetAdapter(view)
            }else{
    ...
    private fun initAdapter() {
        ...
        newsListAdapter = FinanceNewsAdapter { newsViewModel?.retry() }
        recyclerView.layoutManager = LinearLayoutManager(context, LinearLayoutManager.VERTICAL, false)
        recyclerView.adapter = newsListAdapter

        newsViewModel?.newsList?.observe(viewLifecycleOwner, Observer {
            Log.i(TAG, "initAdapter : financeNewsViewModel?.newsList?.observe")
            if(newsViewModel!!.listIsEmpty()){
                Log.d(TAG, "initState financeNewsViewModel!!.listIsEmpty : showErrorLayout : errorMsg : " + newsViewModel?.getErrorMsg())
                showErrorLayout()
                Utils.showToastMessage(context,"Could Not Get Data From API!\nSometimes HTTP 429 Too Many Requests")
            }else{
                Log.d(TAG, "initState financeNewsViewModel!!.listIsEmpty : nofityItemChange and hideErrorLayout")
    ...
    class FinanceNewsAdapter(private val retry: () -> Unit) :
        PagedListAdapter<Article, FinanceNewsAdapter.ViewHolder>(NewsDiffCallback) {

        val TAG: String = FinanceNewsAdapter::class.simpleName.toString()
