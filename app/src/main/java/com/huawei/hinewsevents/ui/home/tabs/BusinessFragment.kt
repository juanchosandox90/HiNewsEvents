...
class BusinessFragment : Fragment() {
    ...
    val TAG: String = BusinessFragment::class.simpleName.toString()
    ...
    private fun initAdapter() {
        ...
        newsListAdapter = BusinessNewsAdapter { newsViewModel?.retry() }
        recyclerView.layoutManager = LinearLayoutManager(context, LinearLayoutManager.VERTICAL, false)
        recyclerView.adapter = newsListAdapter

        newsViewModel?.newsList?.observe(viewLifecycleOwner, Observer {
            Log.i(TAG, "initAdapter : businessNewsViewModel?.newsList?.observe")
            if(newsViewModel!!.listIsEmpty()){
                Log.d(TAG, "initState businessNewsViewModel!!.listIsEmpty : showErrorLayout : errorMsg : " + newsViewModel?.getErrorMsg())
                showErrorLayout()
                Utils.showToastMessage(context,"Could Not Get Data From API!\nSometimes HTTP 429 Too Many Requests")
            }else{
                Log.d(TAG, "initState businessNewsViewModel!!.listIsEmpty : nofityItemChange and hideErrorLayout")
    ...
    class BusinessNewsAdapter(private val retry: () -> Unit) :
        PagedListAdapter<Article, BusinessNewsAdapter.ViewHolder>(NewsDiffCallback) {

        val TAG: String = BusinessNewsAdapter::class.simpleName.toString()
